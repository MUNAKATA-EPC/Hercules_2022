...
// Commands
#define SG12232_STATIC_DRIVE_ON_COMMAND   0xa5
#define SG12232_STATIC_DRIVE_OFF_COMMAND  0xa4
#define SG12232_RESET_COMMAND             0xe2
...
static uint8_t GeneralReadData(uint8_t ctrl, const TPinSet &PinSet)
{
  ...
  // Control A0 pin 
  PinSet.A0_DIPin.output(ctrl&MGLCD_DATA);
...
static uint8_t StandardReadData(uint8_t ctrl, const TPinSet &PinSet)
{
  uint8_t result;

  // Control A0 pin
  if(ctrl&MGLCD_DATA) {
    sbi(&STANDARD_A0_DI_PIN_OUT_REG,STANDARD_A0_DI_PIN_BIT);	
  } else {
...
static void GeneralWriteData(uint8_t ctrl, uint8_t data, const TPinSet &PinSet)
{
  ...
  // control E1 pin
  if(ctrl&MGLCD_CS1) {
    *PinSet.CS1_E1Pin.OutReg|= PinSet.CS1_E1Pin.mask;
    *PinSet.CS1_E1Pin.OutReg&=~PinSet.CS1_E1Pin.mask;
  } // if

  // control E2 pin
  if(ctrl&MGLCD_CS2) {
    *PinSet.CS2_E2Pin.OutReg|= PinSet.CS2_E2Pin.mask;
    *PinSet.CS2_E2Pin.OutReg&=~PinSet.CS2_E2Pin.mask;
  } // if
...
static void StandardWriteData(uint8_t ctrl, uint8_t data, const TPinSet &PinSet)
{
  uint8_t oldSREG;
  ...
  // Control E1/E2 pin 
  if(ctrl&MGLCD_CS1) sbi(&STANDARD_CS1_E1_PIN_OUT_REG,STANDARD_CS1_E1_PIN_BIT);
  if(ctrl&MGLCD_CS2) sbi(&STANDARD_CS2_E2_PIN_OUT_REG,STANDARD_CS2_E2_PIN_BIT);
  __asm__ volatile("nop\n\t"); // to meet access time
  __asm__ volatile("nop\n\t"); // to meet access time
  __asm__ volatile("nop\n\t"); // to meet access time
  __asm__ volatile("nop\n\t"); // to meet access time
...
uint8_t (*const specific_S12232ZA::GeneralReadData)(uint8_t ctrl, const TPinSet &PinSet) = ::GeneralReadData;
uint8_t (*const specific_S12232ZA::StandardReadData)(uint8_t ctrl, const TPinSet &PinSet) = ::StandardReadData;
void (*const specific_S12232ZA::GeneralWriteData )(uint8_t ctrl, uint8_t data, const TPinSet &PinSet) = ::GeneralWriteData;
void (*const specific_S12232ZA::StandardWriteData)(uint8_t ctrl, uint8_t data, const TPinSet &PinSet) = ::StandardWriteData;

void specific_S12232ZA::Reset(void)
{
  ...
  PinSet.RWPin    .output(0);
  WriteData(MGLCD_CS1_CS2_COMMAND, SG12232_RESET_COMMAND,PinSet);
  while(ReadData(MGLCD_CS1_COMMAND,PinSet)&0x90 || ReadData(MGLCD_CS2_COMMAND,PinSet)&0x90); // wait until LCD is ready
  WriteData(MGLCD_CS1_CS2_COMMAND, SG12232_STATIC_DRIVE_OFF_COMMAND,PinSet);
