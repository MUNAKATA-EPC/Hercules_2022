...
thresholds1 = [(8, 100, 39, 82, 9, 71)]#メイン
...
sensor.set_auto_whitebal(False,(-5.02073, -5.119987, 0.3176831))# must be turned off for color tracking
clock = time.clock()
...
while(True):  #メインループ
 ...
 cut_lef_and_rig=170#画面上での縦線の定義
 ...
 for blob in img.find_blobs([thresholds1[threshold_index]], pixels_threshold=1, area_threshold=1, merge=True,margin=25):#下９０行目までカラーボールトラッキング
     ...
     cyA[n]=blob.cy()
     rectA[n]=blob.rect()
     val[n]=blob.cx()-cut_fro_and_bak#値の調整　画面中央を座標(0,0)に
     vall[n]=blob.cy()-cut_lef_and_rig
