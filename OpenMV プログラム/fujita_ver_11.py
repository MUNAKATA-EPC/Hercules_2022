...
thresholds1 = [(35, 100, 40, 72, 13, 41)]#メイン
thresholds2 = [(0, 100, -16, 19, 34, 44)]#ゴール（黄色）
thresholds3 = [(16, 43, 27, 52, -77, -60)]#ゴール（青色）
...
sensor.set_auto_whitebal(False,(-5.02073, -5.019987, 0.576831))# must be turned off for color tracking
clock = time.clock()
...
while(True):  #メインループ
 ...
 cut_lef_and_rig=139]][#画面上での縦線の定義
 ...
 for blob in img.find_blobs([thresholds1[threshold_index]], pixels_threshold=1, area_threshold=1, merge=True,margin=25):#下９０行目までカラーボールトラッキング
     ...
     cyA[n]=blob.cy()
     rectA[n]=blob.rect()
     valA[n]=blob.cy()-cut_fro_and_bak#値の調整　画面中央を座標(0,0)に
     vallA[n]=blob.cx()-cut_lef_and_rig
 ...
 for blob in img.find_blobs([thresholds2[threshold_index]], pixels_threshold=1, area_threshold=1, merge=True,margin=25):
     ...
     cyB[N]=blob.cy()
     rectB[N]=blob.rect()
     valB[N]=blob.cy()-cut_fro_and_bak#値の調整　画面中央を座標(0,0)に
     vallB[N]=blob.cx()-cut_lef_and_rig
 ...
 for blob in img.find_blobs([thresholds3[threshold_index]], pixels_threshold=1, area_threshold=1, merge=True,margin=25):
     ...
     cyC[m]=blob.cy()
     rectC[m]=blob.rect()
     valC[m]=blob.cy()-cut_fro_and_bak#値の調整　画面中央を座標(0,0)に
     vallC[m]=blob.cx()-cut_lef_and_rig
